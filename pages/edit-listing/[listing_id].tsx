...
export default function Page({
  ...
  >(
    previousListing?.items_asking.map((item) => ({
      id: item.item_id,
      name: item.name,
      amount: item.amount,
  ...
  >(
    previousListing?.items_selling.map((item) => ({
      id: item.item_id,
      name: item.name,
      amount: item.amount,
  ...
  const [location, setLocation] = useState<string>(
    previousListing?.listing?.location ?? LOCATIONS_LIST[0]
  );

  const [oncehumanusername, setOncehumanusername] = useState<string>(
    previousListing?.listing.oncehuman_username ?? ""
  );
  const [doNotDiscordContact, setDoNotDiscordContact] = useState(
    previousListing?.listing?.do_not_contact_discord === true
  );
  ...
  const postListing = async () => {
    ...
    try {
      ...
      toast(`Failed to update listing: ${error.message}`);
    } finally {
      showLoading(false);
    }
  };
...
export async function getServerSideProps({ req, res, query, params }) {
  ...
  if (
    !listing ||
    listing.user_info.id !== session.user_id ||
    isNaN(parseInt(listingID, 10))
  ) {
